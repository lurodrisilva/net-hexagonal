...
export const options = (() => {
  ...
  // DocumentDB tier-driven profile. TIER env var picks the per-metal arrival-
  // rate ramp; assumed parallelism is 6 runners (matching testrun-documentdb.yaml).
  // Per-runner target -> aggregate-RPS estimate is target * 6.
  //
  // Sizing rationale: Mongo vCore tier compute roughly doubles per step
  // (M10 0.5 vCPU, M20 1, M30 2, M40 4). The peak arrival rate is set
  // ~2-3× the expected steady-state CPU-bound RPS so the runner *can*
  // saturate the DB; the report tracks the actual saturation reached.
  ...
  return {
    scenarios: {
      ...
      list_heat: {
        ...
        startTime: "30s",
        duration: "2m",
        rate: Math.max(10, Math.round(p.peak * 0.05)),
        timeUnit: "1s",
        preAllocatedVUs: 200,
        maxVUs: 200,
      },
...
function thresholds() {
  // User SLA target for the DocumentDB tier sweep: keep median (p50) under
  // 20 ms while pushing DB CPU/IOPS into the 60-80% band. p95 is left as
  // a soft observability metric (no fail) — the doc explicitly accepts p95
  // climbing past 20 ms once DB enters the saturation band.
  return {
    "http_req_duration{name:create}":       ["med<20"],
    "http_req_duration{name:get}":          ["med<20"],
    "http_req_duration{name:list}":         ["med<20"],
    "http_req_duration{name:update}":       ["med<20"],
